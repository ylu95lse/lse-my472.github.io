...
```{r}
current_dir <- "/Users/luyunjiao/Desktop/MY472 Data for data scientist/W5/Examples/"
```

Reading/parsing the first file into R:

```{r}
w5_example_1_parsed <- fromJSON(paste(current_dir, "w5_example_1.json", sep = ""))
class(w5_example_1_parsed)
names(w5_example_1_parsed)
```
...
```{r}
class(w5_example_1_parsed$supervisor)
```

We can access the supervisor's department with:

```{r}
w5_example_1_parsed$supervisor$department
```
...
```{r}
w5_example_2_parsed <- fromJSON(paste(current_dir, "w5_example_2.json", sep = ""))
class(w5_example_2_parsed)
names(w5_example_2_parsed)
```
...
```{r}
w5_example_3_parsed <- fromJSON(paste(current_dir, "w5_example_3.json", sep = ""))
class(w5_example_3_parsed)
names(w5_example_3_parsed)
```

The dataframe contained in the MT key:

```{r}
w5_example_3_parsed$MT 
```

The dataframe contained in the LT key:

```{r}
w5_example_3_parsed$LT
```
