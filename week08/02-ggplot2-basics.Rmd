...
## Univariate analysis for a single categorical variable 分类变量
...
```{r}
# Base layer
p <- ggplot(df, aes(x = likes_count, y = comments_count)) + xlab("Likes count") x轴标题+
  ylab("Comments count")y轴标题

# Scatter plot: Relationship between number of likes and number of comments
p + geom_point()  

# With smoothed conditional means 平滑趋势线
...
# Box plot and violin plot with log scale 箱体图和小提琴图
p + geom_boxplot() + scale_y_log10(labels = comma)
p + geom_violin() + scale_y_log10(labels = comma)

# Density plot for log like distributions for different parties 密度图
p <- ggplot(df, aes(x = likes_count))
...
p + geom_tile(aes(fill = posts)) 最右边的图例

```


## Multivariate analysis for three continuous variables 多变量
