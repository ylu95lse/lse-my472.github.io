...
p <- p + geom_rect(data=pres, aes(xmin=start, xmax=end, ymin=0, ymax= 0.12, fill=party))+
    scale_fill_manual(values = alpha(c("blue","red"),0.2))
p

datev <- as_date(c("1960-01-01", "1980-01-01", "2000-01-01"))
p <- p + scale_x_date(breaks=c(datev), labels=date_format("%Y"),expand=c(0,0))
p

# Adding president names

p <- p + geom_text(data=pres, aes(x=start,y=0.12,label=name), angle=90) 
p

# Adding lines to separate presidencies
p <- p + geom_vline(data=pres, aes(xintercept=as.numeric(start)),
...
fig <- plot_ly(df_plot, x = ~likes_count, ~shares_count, ~messsage_length, color = ~party)
fig <- fig %>% add_markers()
